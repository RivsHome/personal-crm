...
type View = 'dashboard' | 'calendar' | 'tasks' | 'ideas' | 'finance' | 'goals' | 'gym' | 'diet' | 'movies' | 'music' | 'memories' | 'settings'
...
function App() {
  ...
  const navSections: Array<{ label: string; items: Array<{ id: View; label: string; icon: typeof LayoutDashboard; enabled?: boolean }> }> = [
    ...
    { label: 'Life', items: [{ id: 'goals', label: 'Goals', icon: Target }, { id: 'gym', label: 'Gym', icon: Dumbbell }, { id: 'diet', label: 'Diet Meal Prep', icon: Utensils }, { id: 'memories', label: 'Memories', icon: CircleUserRound }] },
  ...
  return <div className="app-shell" data-theme={preferences?.theme ?? 'dark'} style={{ '--accent': preferences?.accent ?? '#c9f253' } as React.CSSProperties}>
    ...
    <main className="content">
      ...
      {view === 'diet' && <DietModule request={apiFetch} />}
