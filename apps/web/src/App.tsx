...
import { ArrowDown, ArrowUp, CalendarDays, CheckSquare, ChevronRight, CircleUserRound, Download, LayoutDashboard, Plus, Settings, Sparkles, Target, Trash2, WalletCards } from 'lucide-react'
import './module.css'
...
function TaskTree({ tasks, onToggle, onDelete, onAddSubtask }: { tasks: Task[]; onToggle: (id: string) => void; onDelete: (id: string) => void; onAddSubtask: (id: string) => void }) {
  function render(parentId: string | null): React.ReactNode[] {
    return tasks.filter(task => (task.parentId ?? null) === parentId).map(task => <li key={task.id}><button className={`check ${task.completed ? 'done' : ''}`} onClick={() => onToggle(task.id)} title="Toggle task"><CheckSquare size={17} /></button><span className={task.completed ? 'completed' : ''}>{task.summary}</span><button className="subtask" onClick={() => onAddSubtask(task.id)} title="Add subtask"><Plus size={15} /></button><button className="delete" onClick={() => onDelete(task.id)} title="Delete task"><Trash2 size={15} /></button>{tasks.some(child => child.parentId === task.id) && <ul className="task-list nested-tasks">{render(task.id)}</ul>}</li>)
  }
