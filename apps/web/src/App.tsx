...
function App() {
  ...
  const dashboardSchedule = dashboardCalendarDays.flatMap(day => {
    ...
    const calendarItems = events.filter(item => item.date === date).map(item => ({ id: item.id, date, kind: 'event' as const, title: item.title, meta: item.category }))
    const gymItems = gymOccurrencesForDate(day, gymRoutines).map(session => ({ id: `${session.routineId}-${date}`, date, kind: 'gym' as const, title: `Workout ${session.workout}`, meta: session.routineName }))
    ...
    <main className="content">
      ...
      {view === 'dashboard' && <section className="dashboard-grid">
        ...
        {(!preferences || preferences.widgets.calendar) && <article className="panel calendar-panel dashboard-calendar-panel" style={{ order: preferences?.dashboardOrder.indexOf('calendar') ?? 1 }}>
          ...
          <div className="dashboard-week-strip">{dashboardCalendarDays.map((day, index) => { const date = isoDate(day); const dayEvents = events.filter(item => item.date === date); const gymSessions = gymOccurrencesForDate(day, gymRoutines); return <button type="button" className={index === 0 ? 'today' : ''} key={date} onClick={() => { setCalendarMonth(startOfLocalMonth(day)); setView('calendar') }} title={`Open ${day.toLocaleDateString()}`}><span>{day.toLocaleDateString(undefined, { weekday: 'short' })}</span><b>{day.getDate()}</b><i>{dayEvents.length > 0 && <em className="event-dot" />}{gymSessions.length > 0 && <em className="gym-dot" />}</i></button> })}</div>
          {dashboardSchedule.length ? <ul className="dashboard-agenda dashboard-combined-agenda">{dashboardSchedule.slice(0, 4).map(item => <li key={item.id}><button type="button" onClick={() => setView(item.kind === 'gym' ? 'gym' : 'calendar')}><span className={`dashboard-agenda-icon ${item.kind}`} >{item.kind === 'gym' ? <Dumbbell size={14} /> : <CalendarDays size={14} />}</span><span><b>{item.title}</b><small>{new Date(`${item.date}T12:00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} · {item.meta}</small></span><ChevronRight size={14} /></button></li>)}</ul> : <div className="dashboard-calendar-empty"><span>Nothing scheduled for the next seven days.</span><button type="button" onClick={() => setView('calendar')}>Add an event</button></div>}
        </article>}
