...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, CalendarDays, CheckSquare, ChevronDown, ChevronRight, CircleUserRound, Clapperboard, Download, Dumbbell, GripVertical, LayoutDashboard, LogOut, Music2, Pencil, Plus, Search, Settings, Sparkles, Target, Trash2, WalletCards } from 'lucide-react'
import './module.css'
...
type View = 'dashboard' | 'calendar' | 'tasks' | 'ideas' | 'finance' | 'goals' | 'gym' | 'movies' | 'music' | 'memories' | 'settings'
...
function gymOccurrencesForDate(date: Date, routines: GymRoutine[]) {
  const dateValue = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 12).getTime()
  const weekday = date.toLocaleDateString('en-US', { weekday: 'long' }).toLowerCase()
  return routines.flatMap(routine => {
    if (!routine.calendarEnabled) return []
    const start = new Date(`${routine.startDate}T12:00:00`)
    const dayIndex = routine.trainingDays.indexOf(weekday)
...
function App() {
  ...
  useEffect(() => {
    ...
    void apiFetch('/api/gym/routines').then(response => response.ok ? response.json() : []).then(setGymRoutines)
  }, [user])
  ...
  const dashboardCalendarDays = Array.from({ length: 7 }, (_, index) => { const date = new Date(today.getFullYear(), today.getMonth(), today.getDate() + index, 12); return date })
  const dashboardSchedule = dashboardCalendarDays.flatMap(day => {
    const date = isoDate(day)
    const calendarItems = events.filter(item => item.date === date).map(item => ({ id: item.id, date, kind: 'event' as const, title: item.title, meta: item.category, workout: undefined }))
    const gymItems = gymOccurrencesForDate(day, gymRoutines).map(session => ({ id: `${session.routineId}-${date}`, date, kind: 'gym' as const, title: `Workout ${session.workout}`, meta: session.routineName, workout: session.workout }))
    return [...calendarItems, ...gymItems]
  })
  const navSections: Array<{ label: string; items: Array<{ id: View; label: string; icon: typeof LayoutDashboard; enabled?: boolean }> }> = [
    { label: 'Overview', items: [{ id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard }] },
    { label: 'Productivity', items: [
      { id: 'calendar', label: 'Calendar', icon: CalendarDays, enabled: !preferences || preferences.modules.calendar },
      { id: 'tasks', label: 'Tasks', icon: CheckSquare, enabled: !preferences || preferences.modules.tasks },
      { id: 'ideas', label: 'Ideas', icon: Sparkles, enabled: !preferences || preferences.modules.ideas }
    ] },
    { label: 'Finance', items: [{ id: 'finance', label: 'Wallet', icon: WalletCards }] },
    { label: 'Life', items: [{ id: 'goals', label: 'Goals', icon: Target }, { id: 'gym', label: 'Gym', icon: Dumbbell }, { id: 'memories', label: 'Memories', icon: CircleUserRound }] },
  ...
  return <div className="app-shell" data-theme={preferences?.theme ?? 'dark'} style={{ '--accent': preferences?.accent ?? '#c9f253' } as React.CSSProperties}>
    <aside className="sidebar">
      <div className="brand"><span className="brand-mark"><Sparkles size={17} /></span><span><strong>Personal CRM</strong><small>Life, organized</small></span></div>
      <label className="search"><Search size={16} /><input value={moduleQuery} onChange={event => setModuleQuery(event.target.value)} placeholder="Find a module" aria-label="Find a module" /></label>
      <nav aria-label="Main navigation">
        {visibleNavSections.map(section => <div className="nav-section" key={section.label}><p>{section.label}</p>{section.items.map(item => { const Icon = item.icon; return <button type="button" className={`nav-link ${view === item.id ? 'active' : ''}`} aria-current={view === item.id ? 'page' : undefined} onClick={() => { setView(item.id); setModuleQuery('') }} key={item.id}><Icon size={17} /> <span>{item.label}</span>{view === item.id && <span className="active-dot" />}</button> })}</div>)}
        {visibleNavSections.length === 0 && <div className="nav-empty">No modules found</div>}
      </nav>
      <button className="account" onClick={() => void signOut}><span className="avatar">{user.name.trim().charAt(0).toUpperCase()}</span><span><b>{user.name}</b><small>{user.email}</small></span><LogOut size={16} /></button>
    </aside>
    <main className="content">
      <header className="page-header"><div><div className="eyebrow">{`${displayDate.weekday}, ${displayDate.month} ${displayDate.day}, ${displayDate.year}`.toUpperCase()}</div><h1>{view === 'dashboard' ? 'Dashboard' : view[0].toUpperCase() + view.slice(1)}</h1><p>Your personal command center.</p></div><div className="header-actions"><button className="icon-button" onClick={() => void downloadExport()} title="Download data export" aria-label="Download data export"><Download size={18} /></button><button className="icon-button" onClick={() => setView('settings')} title="Open settings" aria-label="Open settings"><Settings size={18} /></button></div></header>
      ...
      {view === 'calendar' && <section className="module-workspace panel">
        <div className="panel-header"><div><span className="panel-kicker"><CalendarDays size={15} /> PLANNING</span><h2>Calendar</h2></div></div>
        <form className="module-form task-editor" onSubmit={createEvent}><input value={eventTitle} onChange={event => setEventTitle(event.target.value)} placeholder="Event title" required /><input type="date" value={eventDate} onChange={event => setEventDate(event.target.value)} required /><input value={eventCategory} onChange={event => setEventCategory(event.target.value)} placeholder="Category" required /><textarea value={eventNotes} onChange={event => setEventNotes(event.target.value)} placeholder="Event notes" /><button type="submit"><Plus size={17} /> {editingEvent ? 'Save event' : 'Add event'}</button>{editingEvent && <button type="button" className="text-action" onClick={() => { setEditingEvent(null); setEventTitle(''); setEventNotes(''); setEventCategory('General') }}>Cancel</button>}</form>
        <div className="calendar-toolbar"><button className="icon-button" onClick={() => setCalendarMonth(current => new Date(current.getFullYear(), current.getMonth() - 1, 1))} title="Previous month"><ArrowLeft size={17} /></button><h3>{calendarMonth.toLocaleString(undefined, { month: 'long', year: 'numeric' })}</h3><button className="icon-button" onClick={() => setCalendarMonth(current => new Date(current.getFullYear(), current.getMonth() + 1, 1))} title="Next month"><ArrowRight size={17} /></button></div>
        {gymRoutines.some(routine => routine.calendarEnabled) && <div className="calendar-sync-legend"><span><Dumbbell size={13} /> Gym routine</span><small>Synced automatically from your training plan</small></div>}
        <div className="month-grid">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => <b key={day} className="month-heading">{day}</b>)}{calendarDays(calendarMonth).map(day => {
          const date = isoDate(day)
          const dayEvents = events.filter(item => item.date === date)
          const gymSessions = gymOccurrencesForDate(day, gymRoutines)
          return <div className={`month-day ${day.getMonth() === calendarMonth.getMonth() ? '' : 'outside'}`} key={date}><span>{day.getDate()}</span>{dayEvents.slice(0, gymSessions.length ? 1 : 2).map(item => <button key={item.id} className="calendar-event" onClick={() => startEditEvent(item)} title={`${item.category}: ${item.notes || item.title}`}>{item.title}</button>)}{gymSessions.slice(0, 1).map(session => <button key={session.routineId} className={`gym-calendar-event gym-calendar-${session.workout.toLowerCase()}`} onClick={() => setView('gym')} title={`${session.routineName} · Workout ${session.workout}`}><Dumbbell size={10} /><span>Gym</span><b>{session.workout}</b></button>)}{gymSessions.length > 1 && <button className="gym-calendar-more" onClick={() => setView('gym')}>+{gymSessions.length - 1}</button>}</div>
        })}</div>
        <h3>Agenda</h3>{monthEvents.length === 0 ? <div className="empty compact"><CalendarDays size={28} /><p>No personal events this month</p>{gymRoutines.some(routine => routine.calendarEnabled) && <small>Your routine sessions are shown quietly in the calendar above.</small>}</div> : <ul className="simple-list">{monthEvents.map(item => <li key={item.id}><span><b>{item.title}</b><small>{item.date} · {item.category}{item.notes ? ` · ${item.notes}` : ''}</small></span><span><button className="subtask" onClick={() => startEditEvent(item)} title="Edit event"><Pencil size={15} /></button><button className="delete" onClick={() => void deleteEvent(item.id)} title="Delete event"><Trash2 size={15} /></button></span></li>)}</ul>}
      </section>}
      ...
      {view === 'gym' && <GymModule request={apiFetch} onRoutinesChange={setGymRoutines} />}
      ...
      {view === 'dashboard' && <section className="dashboard-grid">
        ...
        {(!preferences || preferences.widgets.calendar) && <article className="panel calendar-panel dashboard-calendar-panel" style={{ order: preferences?.dashboardOrder.indexOf('calendar') ?? 1 }}>
          <div className="panel-header"><div><span className="panel-kicker"><CalendarDays size={15} /> NEXT 7 DAYS</span><h2>Calendar</h2></div><button className="icon-button" onClick={() => setView('calendar')} title="Open calendar"><ChevronRight size={17} /></button></div>
          <div className="dashboard-week-strip">{dashboardCalendarDays.map((day, index) => { const date = isoDate(day); const dayEvents = events.filter(item => item.date === date); const gymSessions = gymOccurrencesForDate(day, gymRoutines); const workout = gymSessions[0]?.workout.toLowerCase(); return <button type="button" className={index === 0 ? 'today' : ''} key={date} onClick={() => { setCalendarMonth(startOfLocalMonth(day)); setView('calendar') }} title={`Open ${day.toLocaleDateString()}`}><span>{day.toLocaleDateString(undefined, { weekday: 'short' })}</span><b>{day.getDate()}</b><i>{dayEvents.length > 0 && <em className="event-dot" />}{workout && <em className={`gym-dot workout-color-${workout}`} />}</i></button> })}</div>
          {dashboardSchedule.length ? <ul className="dashboard-agenda dashboard-combined-agenda">{dashboardSchedule.slice(0, 4).map(item => <li key={item.id}><button type="button" onClick={() => setView(item.kind === 'gym' ? 'gym' : 'calendar')}><span className={`dashboard-agenda-icon ${item.kind}${item.workout ? ` workout-color-${item.workout.toLowerCase()}` : ''}`} >{item.kind === 'gym' ? <Dumbbell size={14} /> : <CalendarDays size={14} />}</span><span><b>{item.title}</b><small>{new Date(`${item.date}T12:00:00`).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} · {item.meta}</small></span><ChevronRight size={14} /></button></li>)}</ul> : <div className="dashboard-calendar-empty"><span>Nothing scheduled for the next seven days.</span><button type="button" onClick={() => setView('calendar')}>Add an event</button></div>}
        </article>}
