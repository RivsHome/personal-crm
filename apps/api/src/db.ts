...
function normalizeGymRoutine(row: Record<string, unknown>): GymRoutine {
  ...
  const dayWorkouts = row.dayWorkouts && typeof row.dayWorkouts === 'object' ? row.dayWorkouts as Record<string, WorkoutKey> : {}
  return {
    ...row,
    ...
    scheduleMode: row.scheduleMode === 'manual' ? 'manual' : 'rotation',
    dayWorkouts
  } as GymRoutine
}
...
export async function listGymRoutines(): Promise<GymRoutine[]> {
  ...
  if (!postgresAvailable) routines = localGymRoutines.map(routine => normalizeGymRoutine(routine as unknown as Record<string, unknown>))
    ...
    const result = await pool.query(`SELECT id, name, description, calendar_enabled AS "calendarEnabled", training_days AS "trainingDays", start_date AS "startDate", workout_order AS "workoutOrder", workouts, cardio_minutes AS "cardioMinutes", schedule_mode AS "scheduleMode", day_workouts AS "dayWorkouts", progression,
...
export async function createGymRoutine(input: GymRoutineInput): Promise<GymRoutine> {
  ...
  const result = await pool.query(`INSERT INTO gym_routines (id, name, description, calendar_enabled, training_days, start_date, workout_order, workouts, cardio_minutes, schedule_mode, day_workouts, progression)
    VALUES ($1,$2,$3,$4,$5::jsonb,$6,$7::jsonb,$8::jsonb,$9::jsonb,$10,$11::jsonb,$12::jsonb)
    RETURNING id, name, description, calendar_enabled AS "calendarEnabled", training_days AS "trainingDays", start_date AS "startDate", workout_order AS "workoutOrder", workouts, cardio_minutes AS "cardioMinutes", schedule_mode AS "scheduleMode", day_workouts AS "dayWorkouts", progression, created_at AS "createdAt", updated_at AS "updatedAt"`,
    [routine.id, input.name, input.description, input.calendarEnabled, JSON.stringify(input.trainingDays), input.startDate, JSON.stringify(input.workoutOrder), JSON.stringify(input.workouts), JSON.stringify(input.cardioMinutes), input.scheduleMode, JSON.stringify(input.dayWorkouts), JSON.stringify(input.progression)])
...
export async function updateGymRoutine(id: string, input: GymRoutineInput): Promise<GymRoutine | null> {
  ...
  const result = await pool.query(`UPDATE gym_routines SET name=$2, description=$3, calendar_enabled=$4, training_days=$5::jsonb, start_date=$6,
    workout_order=$7::jsonb, workouts=$8::jsonb, cardio_minutes=$9::jsonb, schedule_mode=$10, day_workouts=$11::jsonb, progression=$12::jsonb, updated_at=NOW() WHERE id=$1 AND user_id='local'
    RETURNING id, name, description, calendar_enabled AS "calendarEnabled", training_days AS "trainingDays", start_date AS "startDate", workout_order AS "workoutOrder", workouts, cardio_minutes AS "cardioMinutes", schedule_mode AS "scheduleMode", day_workouts AS "dayWorkouts", progression, created_at AS "createdAt", updated_at AS "updatedAt"`,
    [id, input.name, input.description, input.calendarEnabled, JSON.stringify(input.trainingDays), input.startDate, JSON.stringify(input.workoutOrder), JSON.stringify(input.workouts), JSON.stringify(input.cardioMinutes), input.scheduleMode, JSON.stringify(input.dayWorkouts), JSON.stringify(input.progression)])
